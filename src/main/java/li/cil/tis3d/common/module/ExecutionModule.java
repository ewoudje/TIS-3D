...
public final class ExecutionModule extends AbstractModuleWithRotation implements ModuleWithBlockChangeListener {
    ...
    private static final class SourceCodeProviderVanilla implements SourceCodeProvider {
        ...
        public @Nullable Iterable<String> codeFor(final ItemStack stack) {
            final WritableBookContent writableBook = stack.get(DataComponents.WRITABLE_BOOK_CONTENT);
            final List<String> code;
            if (writableBook != null) {
                code = writableBook.pages().stream()
                    .map(f -> f.get(false))
                    .map(s -> s.replaceAll("§[a-z0-9]", ""))
                    .flatMap(s -> Arrays.stream(Constants.PATTERN_LINES.split(s)))
                    .toList();
            } else {
                final WrittenBookContent written = stack.get(DataComponents.WRITTEN_BOOK_CONTENT);
                if (written == null) return null;
                code = written.pages().stream()
                    .map(f -> f.get(false))
                    .map(s -> s.getString().replaceAll("§[a-z0-9]", ""))
                    .flatMap(s -> Arrays.stream(Constants.PATTERN_LINES.split(s)))
                    .toList();
            }
